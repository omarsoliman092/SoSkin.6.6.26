...
import { Mail, Lock, ArrowLeft, Sparkles, Eye, EyeOff, User } from "lucide-react";
...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      { title: "Create Account — SoSkin" },
      { name: "description", content: "Create your free SoSkin account and start your AI-powered skincare journey today." },
      { property: "og:title", content: "Create Account — SoSkin" },
      { property: "og:description", content: "Create your free SoSkin account and start your AI-powered skincare journey today." },
    ],
  }),
...
function SignupPage() {
  ...
  const handleSignup = async (e: FormEvent) => {
    ...
    if (!email.trim() || !password || password.length < 6) {
      toast.error(ar ? "الباسورد لازم يكون 6 أحرف على الأقل" : "Password must be at least 6 characters");
      return;
    }
    setLoading(true);
    ...
    toast.success(ar ? "أهلاً بيك في سو سكين!" : "Welcome to SoSkin!");
    navigate({ to: "/" });
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm animate-float-up">
        ...
        <p className="text-sm text-muted-foreground mt-1 mb-6">{t.joinPrompt}</p>
        ...
        <form onSubmit={handleSignup} className="space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={ar ? "الباسورد (6 أحرف على الأقل)" : "Password (min 6 characters)"}
              required
              minLength={6}
          ...
          >
            {loading ? (ar ? "جاري الإنشاء..." : "Creating account...") : t.signUp}
          </button>
        </form>
